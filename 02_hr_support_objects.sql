...
CREATE TABLE HR_DATA_GEN_CONFIG (
    ...
    CONSTRAINT HR_DGEN_CFG_PCT_SUM   CHECK (ROUND(pct_active + pct_on_leave + pct_probation + pct_terminated,2) = 100)
);
...
CREATE TABLE HR_DATA_GEN_RUN_AUDIT (
    ...
    CONSTRAINT HR_DGEN_AUD_ACTION    CHECK (action_type IN ('GENERATE_BATCH','GENERATE_BULK','RESET_TRANSACTIONS','REGENERATE_EMPLOYEES','REVERT_SNAPSHOT','SEED_RECRUITMENT','SEED_TRAINING')),
...
USING (
    ...
    SELECT 'GRATUITY', 'Gratuity Eligibility Verification (Parameterized)', 'GRATUITY', 'PARAMETERIZED', 'Use HR_COMPLIANCE_PKG.GET_COMPLIANCE_REFCURSOR(''GRATUITY'', :P_FROM_DATE,:P_TO_DATE,:P_DEPT_ID,:P_GRADE_ID)' from dual UNION ALL
    SELECT 'NBR_TAX', 'NBR Tax Validation (Parameterized)', 'NBR_TAX', 'PARAMETERIZED', 'Use HR_COMPLIANCE_PKG.GET_COMPLIANCE_REFCURSOR(''NBR_TAX'', :P_FROM_DATE,:P_TO_DATE,:P_DEPT_ID,:P_GRADE_ID)' from dual UNION ALL
    SELECT 'TURNOVER', 'Turnover Analysis (Parameterized)', 'TURNOVER', 'PARAMETERIZED', 'Use HR_COMPLIANCE_PKG.GET_COMPLIANCE_REFCURSOR(''TURNOVER'', :P_FROM_DATE,:P_TO_DATE,:P_DEPT_ID,:P_GRADE_ID)' from dual UNION ALL
    SELECT 'SALARY_AUDIT', 'Salary Audit (Parameterized)', 'SALARY_AUDIT', 'PARAMETERIZED', 'Use HR_COMPLIANCE_PKG.GET_COMPLIANCE_REFCURSOR(''SALARY_AUDIT'', :P_FROM_DATE,:P_TO_DATE,:P_DEPT_ID,:P_GRADE_ID)' from dual
) s
